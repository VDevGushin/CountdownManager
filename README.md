...
5. Нажмите кнопку редактирования события: подзадачи, emoji и удаление с подтверждением доступны в одном редакторе.

Изменения редактора попадают в данные только после **«Сохранить»**. **«Отмена»** отбрасывает черновик. Окно оформлено как компактная utility-поверхность без обычного заголовка и traffic-light controls. **Command-W**, переход в другое приложение или на другой Space скрывают окно, сохраняя в памяти позицию списка, открытый редактор и введённый текст. Нажатие строки меню показывает тот же сеанс на текущем active Space, а если окно уже видно — скрывает его. Выход из приложения может сбросить несохранённые черновики.

Автозапуск включается флажком **«Запускать при входе в macOS»** в нижней части окна. По умолчанию он выключен.
...
Когда событий нет, окно показывает короткое пустое состояние с кнопкой **+ Добавить событие**, а строка меню сразу возвращается к **◷ Countdown**.
