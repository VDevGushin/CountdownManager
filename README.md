...
- Индикатор `▸ 2/8` или `▾ 2/8` показывает количество выполненных и общее количество. Состояние сворачивания хранится отдельно для каждого события и восстанавливается после перезапуска.
...
- Когда время истекает, таймер переходит в состояние `⏰ Время вышло` и остаётся в нём до удаления: играет встроенный звук, на пять секунд появляется собственное неактивирующее окно приложения поверх остальных окон, а значок будильника в строке меню покачивается (при включённом системном «Уменьшении движения» — пульсирует).
...
Пока таймер работает, его показание временно занимает строку меню: `⏰ 29:59`, а после завершения — просто `⏰`, пока таймер не удалён. Удаление таймера возвращает обычное отображение основного события.
