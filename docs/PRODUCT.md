...
The editor contains the existing event fields, a plain display of the current emoji, compact presets and an `Ещё…` action that opens the standard macOS Character Viewer. The current emoji is not presented as an editable text field. Any single valid emoji can be selected; the presets are shortcuts rather than the allowed set. Event deletion uses an explicit inline confirmation with a cancel action. Launch at login, diagnostics and Quit remain accessible through ordinary controls. A dedicated Restart action is removed; the application can be quit and launched normally.
